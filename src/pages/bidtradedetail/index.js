  ...
  viewModel = {
    ...
    inquiryTypeOpt: function (v) {
      ...
      const inquiryType = {
        '1': '<div>广泛征集供应商报价</div>',
        '2': '<div>发布到我的采购门户</div>',
        '3': '<div id="id1">邀请指定供应商报价&nbsp&nbsp&nbsp</div>'
      }
      let text = arr.map(function (item) {
...
(async function () {
  ...
  bidtradedef.on('data', function (data) {
    ...
    if (type == 1) {
      $('#signId').before('<button class="btn btn-primary btn-sm  m-r-sm" data-bind="click:$root.sign">报名</button>')
    }
    if (data.bidtradeSupplierList) {
      for (var i = 0; i < data.bidtradeSupplierList.length; i++) {
        $('#id1').append('<span style="background-color:#1baede">' + data.bidtradeSupplierList[i].supplierName + '</span>&nbsp&nbsp&nbsp')
      }
    }
  })
  init()
