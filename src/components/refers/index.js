...
/* global ko $ */
function init ({type, searchlist, delay = 300, showtext, showeachtext, value, placeholder = '请输入要查询的内容', showrefer = function () { }, onselect = function () {}}) {
  let defaultValue = ''
  // 初始化
  if (searchlist) {
  ...
  // 输入框中查询key
  this.queryvalue = ko.observableArray([])
  // 查询返回的列表
  ...
  this.showtext.subscribe(function (val) {
    ...
    if (val) {
      ...
      //   this.queryvalue(data.supervisorName)
      // })
      for (var i = 0; i < val.length; i++) {
        console.log('yusn', val[i].supervisorName)
        realVal += val[i].supervisorName + ' '
      }
    }
    this.queryvalue(realVal)
    // if (val) {
    //   console.log('yusn---', val)
    //   this.queryvalue(val)
    // }
  }.bind(this))
  // 鼠标移入输入框事件
